...
def test_km(usual_setup_usual_teardown):
    ...
    _, *data, footer = run_rmlint('-k ' + search_paths, use_default_dir=False)
    assert len(data) == 4 + 3
    ...
    _, *data, footer = run_rmlint(' -m ' + search_paths, use_default_dir=False)
    assert len(data) == 4 + 2
    ...
    #     should also treat 2 of the 'xxx' as originals since two are tagged
    _, *data, footer = run_rmlint(' -km ' + search_paths, use_default_dir=False)
    assert len(data) == 4
