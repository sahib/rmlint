...
def test_keep_hardlinks(usual_setup_usual_teardown):
    ...
    _, *data, _ = run_rmlint('--no-hardlinked -S a')
    ...
    _, *data, _ = run_rmlint('--hardlinked -S a')
    assert data[0]["path"].endswith("file_a")
    assert data[0]["is_original"] is True
    ...
    _, *data, _ = run_rmlint('--keep-hardlinked -S a')
    assert data[0]["path"].endswith("file_a")
    assert data[0]["is_original"] is True
    assert data[1]["path"].endswith("file_b")
...
def test_keep_hardlinks_multiple_originals():
    ...
    _, *data, _ = run_rmlint('--hardlinked -k -m -S a ' + search_paths, use_default_dir=False)
    ...
    _, *data, _ = run_rmlint('--keep-hardlinked -k -m -S a ' + search_paths, use_default_dir=False)
