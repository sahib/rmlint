...
static void rm_cfg_start_gui(int argc, const char **argv) {
...
static int rm_cfg_maybe_switch_to_gui(int argc, const char **argv) {
    for(int i = 0; i < argc; i++) {
        if(g_strcmp0("--gui", argv[i]) == 0) {
            argv[i] = "shredder";
            rm_cfg_start_gui(argc - i - 1, &argv[i + 1]);
...
bool rm_cfg_parse_args(int argc, char **argv, RmCfg *cfg) {
     ...
     */
    if(rm_cfg_maybe_switch_to_gui(argc, (const char **)argv) == EXIT_FAILURE) {
