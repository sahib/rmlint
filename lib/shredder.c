...
static gint32 rm_shred_get_read_size(RmFile *file, RmMainTag *tag) {
    ...
    {
        bytes_to_next_fragment =
            rm_offset_bytes_to_next_fragment(file->disk_offsets, file->seek_offset);
    }
...
static gboolean rm_shred_sift(RmFile *file, bool waiting);
...
static void rm_shred_file_preprocess(_U gpointer key, RmFile *file, RmMainTag *main) {
    ...
    rm_shred_group_push_file(group, file, true, false);
 ...
 * */
static void rm_shred_unbuffered_read_factory(RmFile *file, RmShredDevice *device, gboolean waiting) {
    gint32 bytes_read = 0;
    gint32 total_bytes_read = 0;
