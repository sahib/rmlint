...
static gboolean rm_cmd_parse_permissions(
    _U const char *option_name, const gchar *perms, RmSession *session, GError **error
) {
    RmCfg *cfg = session->cfg;
    ...
    if(perms == NULL) {
        ...
        while(*perms) {
            switch(*perms++) {
                case 'r': cfg->permissions |= R_OK; break;
                case 'w': cfg->permissions |= W_OK; break;
                case 'x': cfg->permissions |= X_OK; break;
                default:
                    g_set_error(
                        error, RM_ERROR_QUARK, 0,
