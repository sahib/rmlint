...
static int blake2bp_init_leaf(blake2b_state *S, size_t outlen, size_t keylen,
                              uint64_t offset) {
    blake2b_param P[1];
...
int blake2bp(void *out, size_t outlen, const void *in, size_t inlen, const void *key,
             size_t keylen) {
    uint8_t hash[PARALLELISM_DEGREE][BLAKE2B_OUTBYTES];
...
int main(void) {
    ...
    for(step = 1; step < BLAKE2B_BLOCKBYTES; ++step) {
        for(i = 0; i < BLAKE2_KAT_LENGTH; ++i) {
            ...
            if((err = blake2bp_init_key(&S, BLAKE2B_OUTBYTES, key, BLAKE2B_KEYBYTES)) <
               0) {
                goto fail;
            }
