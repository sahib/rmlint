...
:``-S --rank-by=criteria`` (**default\:** *pOma*):
    ...
    **r** and **x** which expect a regex pattern after the letter). Multiple
    ...
    Alphabetical sort [aA] will only use the basename of the file and ignore
    its case. Alphabetical sort on dir [fF] uses the full path (excluding
    basename) and is case-sensitive. One can have multiple criteria, e.g.:
    ``-S am`` will choose first alphabetically; if tied then by mtime.
    **Note:** original path criteria (specified using ``//``) will always take
...
:``-U --hash-unmatched``:
    ...
    Fully hash and include in the output those files that had at least one
    size twin but turned out not to be duplicates.

:``--hash-uniques``:

    Like ``--hash-unmatched``, but fully hash *every* traversed file,
    including files with a unique size that would normally never be read.
...
:``--write-unfinished``:
    ...
    The emitted checksums are *partial*: they only cover the part of the file
    that was read before the file was ruled out as a duplicate. Please refer to
    ``--hash-unmatched`` for a variant that finishes the checksums instead.
...
:``rmlint --is-reflink [-v|-V] <file1> <file2>``:
    ...
    * 1: Files are not linked
    * 3: Not a regular file
    * 4: File sizes differ
    * 5: Maybe a reflink (fiemap data could not be read)
